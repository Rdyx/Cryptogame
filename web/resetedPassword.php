...
if(!isset($_SESSION['nick']) || $_SESSION['nick'] === 'Guest') {
    if (!empty($_POST['nick']) && !empty($_POST['newPwd']) && !empty($_POST['newPwd2']) && !empty($_POST['pinCode'])) {
        ...
        $result = mysqli_query($conn, "SELECT * FROM cry_users WHERE usr_name = '$nick'");
        $row = $result->fetch_assoc();

        if (password_verify($pinCode, $row['usr_pinCode'])) {
            if ($newPwd === $newPwd2) {
            ...
            <div class="container-fluid black-div underTopDiv">
                ...
                <div>
                    <p class="mt-5">Vous vous êtes trompé quelque part !</p>
                    <p class="mt-5">Vous allez être redirigé automatiquement dans 3 secondes.</p>
                    <p>Si rien ne se passe, <a href="/web/forgotPassword.php">vous pouvez cliquer ici.</a></p>
                </div>
            </div>
